...
impl crate::plotters_render::PlottersRenderable for BoxPlot {
    fn render_plotters(
        &self,
        ...
        bridge::render_plotters_to_buf(
            ...
            |root| {
                ...
                for (i, bd) in data_ref.iter().enumerate() {
                    let (q1, median, q3) = bd.quartiles();
                    let (whisker_lo, whisker_hi) = bd.whiskers();
                    let x = i as f64;
                    let half_w = 0.35;
                    let pc = theme_bridge::to_plotters_color(bd.color);

                    // Draw box (Q1 to Q3)
                    let _ = chart.draw_series(std::iter::once(
                        plotters::element::Rectangle::new(
                            [(x - half_w, q1), (x + half_w, q3)],
                    ...
                    let _ = chart.draw_series(std::iter::once(
                        plotters::element::Rectangle::new(
                            [(x - half_w, q1), (x + half_w, q3)],
                            plotters::style::ShapeStyle::from(fg).stroke_width(1),
                        ),
                    ));
                    ...
                    let _ = chart.draw_series(
                        plotters::series::LineSeries::new(
                            vec![(x - half_w, median), (x + half_w, median)],
                    ...
                    let _ = chart.draw_series(
                        plotters::series::LineSeries::new(
                            vec![(x - half_w * 0.5, whisker_lo), (x + half_w * 0.5, whisker_lo)],
                            plotters::style::ShapeStyle::from(fg).stroke_width(1),
                        ),
                    );
                    ...
                    let _ = chart.draw_series(
                        plotters::series::LineSeries::new(
                            vec![(x - half_w * 0.5, whisker_hi), (x + half_w * 0.5, whisker_hi)],
                            plotters::style::ShapeStyle::from(fg).stroke_width(1),
                        ),
                    );
                    ...
                    if self.show_outliers {
                        let outliers = bd.outliers();
                        let _ = chart.draw_series(outliers.iter().map(|&v| {
                            plotters::element::Circle::new(
                                (x, v), 3,
                                plotters::style::ShapeStyle::from(fg).filled(),
                            )
                        }));
                    }
                }

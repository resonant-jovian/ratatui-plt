...
impl crate::plotters_render::PlottersRenderable for BarChart {
    fn render_plotters(
        &self,
        ...
        if self.categories.is_empty() || self.datasets.is_empty() {
            return;
        }
        ...
        // Compute y-bounds across all datasets
        let mut y_max = 0.0f64;
        let mut y_min = 0.0f64;
        match self.mode {
            BarMode::Stacked | BarMode::DivergingStacked => {
                for ci in 0..n_cats {
                    let mut pos_sum = 0.0f64;
                    let mut neg_sum = 0.0f64;
                    for ds in &self.datasets {
                        let v = ds.values.get(ci).copied().unwrap_or(0.0);
                        if v >= 0.0 { pos_sum += v; } else { neg_sum += v; }
                    }
                    y_max = y_max.max(pos_sum);
                    y_min = y_min.min(neg_sum);
                }
            }
            BarMode::Grouped => {
                for ds in &self.datasets {
        ...
        bridge::render_plotters_to_buf(
            ...
            |root| {
                let Ok(mut chart) = helpers::build_cartesian_2d(
                    root, &x_axis, &y_axis, title_ref, theme,
                    -0.5..(n_cats as f64 - 0.5), y_min..y_max,
                ) else { return; };

                // Draw bars as rectangles
                match self.mode {
                    BarMode::Grouped => {
                        let bar_width = 0.8 / n_ds as f64;
                        for (di, ds) in self.datasets.iter().enumerate() {
                            let pc = theme_bridge::to_plotters_color(ds.color);
                            let _ = chart.draw_series(
                                ds.values.iter().enumerate().map(|(ci, &v)| {
                                    let x0 = ci as f64 - 0.4 + di as f64 * bar_width;
                    ...
                    BarMode::Stacked | BarMode::DivergingStacked => {
                        // Track cumulative positive and negative per category
                        let mut pos_base = vec![0.0f64; n_cats];
                        let mut neg_base = vec![0.0f64; n_cats];
                        for ds in &self.datasets {
                            let pc = theme_bridge::to_plotters_color(ds.color);
                            let _ = chart.draw_series(
                                ds.values.iter().enumerate().map(|(ci, &v)| {
                                    let (y0, y1) = if v >= 0.0 {
                                        ...
                                        let base = neg_base[ci];
                                        neg_base[ci] += v;
                                        (base + v, base)
                                    };
                                    let x0 = ci as f64 - 0.4;
                                    let x1 = ci as f64 + 0.4;
                                    plotters::element::Rectangle::new(
                                        [(x0, y0), (x1, y1)],
                                        plotters::style::ShapeStyle::from(pc).filled(),
                                    )

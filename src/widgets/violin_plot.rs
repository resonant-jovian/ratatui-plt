...
impl ViolinData {
    ...
    fn finite_sorted(&self) -> Vec<f64> {
        let mut v: Vec<f64> = self.values.iter().copied().filter(|x| x.is_finite()).collect();
...
fn gaussian_kde(sorted_values: &[f64], eval_points: &[f64]) -> Vec<f64> {
    ...
    let variance = sorted_values.iter().map(|&x| (x - mean) * (x - mean)).sum::<f64>() / nf;
...
///
/// ```
/// use ratatui_sim::widgets::violin_plot::{ViolinPlot, ViolinData};
    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        for (i, d) in self.datasets.iter().enumerate() {
            ...
            for row in 0..ph {
                ...
                // bottom half-pixel
                let bot_eval_idx_f = (1.0 - (row as f64 * 2.0 + 1.0) / (n_eval as f64 - 1.0).max(1.0))
                    * (n_eval - 1) as f64;
            ...
            if self.show_box {
                    ...
                    if y >= py && y < py + ph {
                        for dx in 0..=box_half {
                            let positions: Vec<u16> = if dx == 0 {
                                ...
                                let mut p = Vec::new();
                                if center_x + dx < px + pw { p.push(center_x + dx); }
                                if center_x >= dx + px { p.push(center_x - dx); }
                                p
                            };
                            for &sx in &positions {
                                if sx >= px && sx < px + pw {
                                    buf[(sx, y)].set_char('│').set_fg(Color::White);
                                }
                            }
                        }
                    }
                ...
                for dx in 0..=box_half {
                    if sy_median >= py && sy_median < py + ph {
                        ...
                        for &sx in &positions {
                            if sx >= px && sx < px + pw {
                                buf[(sx, sy_median)].set_char('━').set_fg(Color::White);
                            }
                        }
                    }
                }

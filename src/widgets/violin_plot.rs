...
impl crate::plotters_render::PlottersRenderable for ViolinPlot {
    fn render_plotters(
        &self,
        ...
        // Compute global y range from all datasets
        ...
        let x_axis = crate::axis::Axis::new();
        ...
        bridge::render_plotters_to_buf(
            ...
            |root| {
                let Ok(mut chart) = helpers::build_cartesian_2d(
                    root, &x_axis, &self.y_axis, title_ref, theme,
                    -0.5..(n as f64 - 0.5), y_lo..y_hi,
                ) else { return; };

                let fg = theme_bridge::fg_color(theme);
                let n_kde = 64;

                for (i, vd) in datasets_ref.iter().enumerate() {
                    let sorted = vd.finite_sorted();
                    ...
                    let eval_lo = data_min - pad;
                    let eval_hi = data_max + pad;
                    let eval_points: Vec<f64> = (0..n_kde)
                        .map(|j| eval_lo + (eval_hi - eval_lo) * j as f64 / (n_kde - 1) as f64)
                        .collect();

                    let kde = gaussian_kde(&sorted, &eval_points);
                    let max_density = kde.iter().cloned().fold(0.0f64, f64::max);
                    if max_density <= 0.0 { continue; }

                    let x_center = i as f64;
                    let half_width = 0.4;

                    let pc = theme_bridge::to_plotters_color(vd.color);
                    let pc_alpha = theme_bridge::to_plotters_color_alpha(vd.color, 0.5);

                    // Build violin polygon: right side top-to-bottom, left side bottom-to-top
                    let mut polygon_points: Vec<(f64, f64)> = Vec::with_capacity(n_kde * 2);
                    // Right side
                    for j in 0..n_kde {
                        let w = (kde[j] / max_density) * half_width;
                        polygon_points.push((x_center + w, eval_points[j]));
                    }
                    // Left side (reversed)
                    for j in (0..n_kde).rev() {
                        let w = (kde[j] / max_density) * half_width;
                        polygon_points.push((x_center - w, eval_points[j]));
                    }
                    ...
                    let _ = chart.draw_series(std::iter::once(
                        plotters::element::Polygon::new(
                            polygon_points,
                            plotters::style::ShapeStyle::from(pc_alpha).filled(),
                        ),
                    ));
                    ...
                    let _ = chart.draw_series(
                        plotters::series::LineSeries::new(
                            vec![(x_center - half_width * 0.3, median), (x_center + half_width * 0.3, median)],
                            plotters::style::ShapeStyle::from(fg).stroke_width(2),
                        ),
                    );

                    // Draw inner box (Q1-Q3) if show_box
                    if self.show_box {
                        let box_hw = half_width * 0.15;
                        let _ = chart.draw_series(std::iter::once(
                            plotters::element::Rectangle::new(
                                [(x_center - box_hw, q1), (x_center + box_hw, q3)],

...
fn main() -> color_eyre::Result<()> {
    ...
    let normal = ViolinData::new(
        "Normal",
        generate_data(42, 800, 5.0, 2.0),
        cycle.next_color(),
    );
    let bimodal = ViolinData::new("Bimodal", bimodal_data(123, 800), cycle.next_color());
    let skewed = ViolinData::new(
        "Skewed",
        generate_data(999, 800, 3.0, 1.0),
        cycle.next_color(),
    );
    let wide = ViolinData::new(
        "Wide",
        generate_data(777, 800, 5.0, 5.0),
        cycle.next_color(),
    );
